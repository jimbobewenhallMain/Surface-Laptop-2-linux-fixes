...
static int ov9734_probe(struct i2c_client *client)
{
	...
	/*
	 * v4l2_i2c_subdev_init() must run before ov9734_power_on(): the
	 * power callbacks look up driver state via i2c clientdata, which
	 * is only set here. Power-on first = near-NULL dereference
	 * (kernel oops at ov9734_power_on+0x17).
	 */
	...
	/*
	 * A pm_runtime_put() here underflows the usage count
	 * (pm_runtime_set_active() does not take a reference).
	 * pm_runtime_idle() has the intended effect - allow the
	 * device to suspend - without the underflow.
	 */
